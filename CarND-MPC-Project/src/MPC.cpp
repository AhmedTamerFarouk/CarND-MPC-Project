...
const double ref_V = 70; // convert from mph to m/s
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int i = 0; i < N; ++i) {
      fg[0] += 7500*CppAD::pow(vars[cte_start + i] - ref_cte, 2);
      fg[0] += 7000*CppAD::pow(vars[epsi_start + i] - ref_epsi, 2);
    ...
    for (unsigned int t = 1; t < N; t++) {
      ...
      /*
      // use previous actuations (to account for latency of 100ms since dt = 0.1
      if (t > 1) {   
        a = vars[a_start + t - 2];
